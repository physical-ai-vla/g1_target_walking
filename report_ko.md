...
**프로젝트 개요.** RGB-D 프레임 + 고유감각(proprioceptive) 센서 + 그 이력 + 고정된 영문 지시문을
...
- **제어 (어떻게 움직일까 — how to move)** — **속도 조건부 PPO 보행 정책**("척추, spine")으로,
  ...
  이 정책의 행동(action)은 **그 자체가 하체 PD 관절 위치 목표** — 즉 이 문제가 요구하는 출력 형태와 정확히 일치한다.
...
즉 네트워크 출력은 문자 그대로 이 문제가 요구하는 관절 목표 벡터이며, 속도나 상위 레벨 핸들이 아니다.

---

## 2.4 왜 VLM(zero-shot) + RL 체크포인트인가 — end-to-end로 학습한 VLA가 아니라

목표는 "작은 VLA"로 이 문제를 푸는 것이었다. 다만 그 VLA를 단일 픽셀→관절 네트워크를 파인튜닝하는 대신,
...
*"궤적과 지시문을 어떻게 생성했고 대략 몇 개인가"* 라는 자연스러운 질문에 대해 — 우리는 **궤적을
0개 수집**했고 **데이터셋 없이** 학습했다; 파이프라인의 어느 부분도 데이터 기반이 아니다:
